...
const Leadership: React.FC = () => {
  ...
  return (
    <section className="bg-[#f1f2fc] py-32 dark:bg-slate-900  text-center bg-fixed">
      <div className="max-w-7xl mx-auto">
        <hr className="border-t border-gray-300 mb-6 mx-20" />
        <div className="flex flex-col items-center">
          <h2 className="text-3xl font-bold font-serif text-[#F4B24A] my-5 text-center">LEADERSHIP</h2>
        </div>
        <div className="flex flex-col md:flex-row justify-center items-center max-w-4xl mx-auto">
          <div className="w-full md:w-1/2 flex flex-col justify-center items-center">
            ...
            <div className="mt-6 text-center">
              <h5 className="text-xl font-bold dark:text-white text-[#02032d]">Mr. Gichengo</h5>
          ...
          <div className="w-full md:w-1/2 mt-10 md:mt-0 px-4 md:px-0">
            <p className="text-md md:text-lg dark:bg-gray-800 md:shadow-none shadow-[2px_2px_4px_rgba(0,0,0,0.1)] rounded p-4 text-left font-sans">
          ...
          <Slider ref={sliderRef} {...settings}>
            {trustees.map((trustee, idx) => (
              <div key={idx} className="px-2">
                <div className="bg-white dark:bg-gray-300  rounded-xl shadow-md p-4 flex flex-col items-center text-center h-full">
                  <img
                    src={trustee.image}
                    alt={trustee.name}
                    className="w-32 h-32 object-cover rounded-full mb-4"
                  />
                  <p className="font-semibold dark:text-[#02032d] text-md">{trustee.name}</p>
                  <div className="mt-2 bg-[#02032d]  text-white text-xs px-4 py-2 rounded-lg">
                    <p className="font-bold text-md">{trustee.title}</p>
                    <p className="text-sm">{trustee.school}</p>
                  </div>
                </div>
              </div>
            ))}
