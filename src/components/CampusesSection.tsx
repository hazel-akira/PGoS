...
const campuses = [
  {
    ...
    desc: "Pioneer school , Pioneer Junior & St Paul Thomas Academy",
...
export default function CampusesSection() {
  return (
    <div className="w-full flex pt-10 mt-10 flex-col md:flex-row justify-between bg-white rounded-b-2xl shadow-md pb-10  px-6 md:px-12 gap-8 md:gap-0 animate-fade-in">
      {campuses.map((campus, idx) => (
        <div key={campus.name} className={`flex-1 flex flex-col items-center pb-4 ${idx < campuses.length - 1 ? "md:border-r border-gray-300" : ""}`}>
          ...
          <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-1 text-sm shadow cursor-pointer border hover:bg-gray-100 transition hover-scale">
            <Phone className="w-5 h-5 text-[#182858]" />
            <span className="underline text-black font-medium">{campus.phone}</span>
          </div>
        </div>
      ))}
    </div>
